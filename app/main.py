...
from app.rbac import list_roles, role_exists, add_role, ROLE_PERMISSIONS
...
def get_roles(token: str):
    """List all roles and the document namespaces each can access."""
    token_data = decode_token(token)
    if not token_data or token_data.role != "admin":
        raise HTTPException(status_code=403, detail="Admin access required")
    return [
        {"role": r, "namespaces": ROLE_PERMISSIONS.get(r, [])}
        for r in list_roles()
    ]


@app.post("/admin/roles", response_model=CreateRoleResponse)
def create_role(request: CreateRoleRequest):
    """Create a new role in the RBAC policy matrix."""
    token_data = decode_token(request.token)
    if not token_data or token_data.role != "admin":
        raise HTTPException(status_code=403, detail="Admin access required")

    try:
        role_name = add_role(request.role_name, request.namespaces)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))

    audit.log_query(
        token_data.username, token_data.role,
        f"[CREATE_ROLE] {role_name}",
        response_type="admin",
    )
    return CreateRoleResponse(
        success=True,
        role_name=role_name,
        namespaces=ROLE_PERMISSIONS.get(role_name, []),
    )
...
def create_new_user(request: CreateUserRequest):
    ...
    if not role_exists(request.role):
        raise HTTPException(
            status_code=400,
            detail=f"Role '{request.role}' does not exist. Create the role first."
        )
    ...
    audit.log_query(
        token_data.username, token_data.role,
        f"[CREATE_USER] {user['username']} → role:{user['role']}",
        response_type="admin",
    )
