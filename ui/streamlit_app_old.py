...
with st.sidebar:
    ...
    if role == "admin":
        upload_role = st.selectbox(
            "Assign to role",
            options=["hr", "finance", "engineering", "marketing", "admin"],
            index=["hr","finance","engineering","marketing","admin"].index(role),
        )
    else:
        upload_role = role
...
if st.session_state.role == "admin":
    ...
    tab1, tab2 = st.tabs(["📋 Audit Logs", "📦 Document Collections"])
